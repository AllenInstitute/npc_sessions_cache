...
Data: TypeAlias = Union[Mapping, plt.Figure, str, upath.UPath]
...
class QCElement:
    ...
    def get_filename(
        self, stem_suffix: str = "", type_suffix: str | None = None
    ) -> str:
        if not type_suffix:
            if isinstance(self.data, str):
                type_suffix = (
                    ".txt" if not self.data.startswith("Traceback") else ".error"
                )
        ...
        **kwargs,
        ...
        if not filename:
            filename = self.get_filename(
                stem_suffix=stem_suffix, type_suffix=type_suffix
            )
        ...
        if isinstance(self.data, str):
            ...
            if not path.protocol:
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    self.data.savefig(
                        (f := pathlib.Path(tmpdir) / path.name), **kwargs | {"dpi": 450}
                    )
...
class QCStore(collections.abc.Mapping):
    ...
    def write_data(
        ...
        for idx, element in enumerate(data):
            ...
            path = element.write(
                self.path, stem_suffix=f"_{idx}" if len(data) > 1 else ""
            )
            paths.append(path)
    ...
    def __getitem__(
        self, key: str | npc_session.SessionRecord
    ) -> tuple[upath.UPath, ...]:
        key = self.normalize_key(key)
        ...
        if key in self._missing:
            logger.debug(
                f"{key} in 'missing' list: previously established that data does not exist on disk"
            )
            raise KeyError(f"{key} qc data not found")
        if key in self._errored:
            logger.debug(
                f"{key} is in 'errored' list: previously established that data is an error"
            )
        ...
        if not paths:
            self._missing.add(key)
            logger.debug(
                f"Data for {key} does not exist on disk: added to 'missing' list"
            )
...
def get_qc_module_names() -> tuple[str, ...]:
    ...
    return tuple(
        ...
        if "utils" not in path.stem.split("_") and not path.stem.startswith("_")
    )


def get_qc_functions(
    module_name: str | None = None,
) -> dict[tuple[str, str], FunctionType]:
    ...
    for m in modules:
        ...
        for name in dir(module):
            ...
            if not (
                ...
                or isinstance(callable_obj, functools.partial)
            ):
                continue
...
def write_output_from_single_function(
    ...
    if skip_existing and key in store:
        logger.info(
            f"Skipping {module_name}.{function_name} for {key} - data already exists"
        )
        return
    if skip_previously_failed and store.is_errored(key):
        logger.info(
            f"Skipping {module_name}.{function_name} for {key} - previously failed to write data"
        )
        return
...
def write_instructions_for_qc_item(
    ...
    if instructions := getattr(module, "instructions", {}):
        ...
        logger.debug(
            f"Writing instructions for {module_name}.{function_name} to {path}"
        )
