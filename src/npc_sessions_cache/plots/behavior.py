...
def plot_performance_by_block(
    ...
    n_passing_blocks = np.sum(
        task_performance_by_block_df["cross_modality_dprime"] >= dprime_threshold
    )
    failed_block_ind = (
        task_performance_by_block_df["cross_modality_dprime"] < dprime_threshold
    )
    ...
    ax[0].plot(
        xvect,
        task_performance_by_block_df["signed_cross_modality_dprime"],
        "ko-",
        label="cross-modal",
    )
    ax[0].plot(
        xvect[failed_block_ind],
        task_performance_by_block_df["signed_cross_modality_dprime"][failed_block_ind],
        "ro",
        label="failed",
    )
    ax[0].axhline(0, color="k", linestyle="--", linewidth=0.5)
    max_dprime = np.nanmax(
        np.abs(task_performance_by_block_df["signed_cross_modality_dprime"])
    )
    ...
    ax[1].plot(xvect, task_performance_by_block_df["vis_dprime"], "go-", label="vis")
    ax[1].plot(xvect, task_performance_by_block_df["aud_dprime"], "bo-", label="aud")
    max_dprime = np.nanmax(
        np.abs(
            np.concatenate(
                [
                    task_performance_by_block_df["vis_dprime"],
                    task_performance_by_block_df["aud_dprime"],
                ]
            )
        )
    )
...
def plot_response_rate_by_stimulus_type(
    ...
    for stim_type in stim_types:
        ...
        response_times = trials[trials[f"is_{stim_type}"] & trials["is_response"]][
            "stim_start_time"
        ].values
    ...
    aud_block_inds = (
        np.arange(0, len(switch_starts), 2) + trials.iloc[0]["is_vis_rewarded"]
    )
    ...
    for aud_block in aud_block_inds:
        rectangle = Rectangle(
            (switch_starts[aud_block], 0),
            switch_durations[aud_block],
            1,
            color="k",
            alpha=0.2,
        )
...
def plot_licks_by_block(session: npc_sessions.DynamicRoutingSession) -> plt.Figure:
    ...
    f = session.task_data
    f["trialPreStimFrames"][:]
    f["preStimFramesFixed"][()]
    ...
    for bb in range(0, len(unique_blocks)):
        ...
        for tt in range(blockTrialStart, blockTrialEnd):
            ...
            if len(lickFrames) > 0:
                ...
                temp_licks = (
                    temp_licks[
                        (temp_licks > temp_start_frame) & (temp_licks < temp_end_frame)
                    ]
                    - trialStimStartFrame[tt]
                )
            ...
            if len(rewardFrames) > 0:
                ...
                temp_reward = (
                    temp_reward[
                        (temp_reward > temp_start_frame)
                        & (temp_reward < temp_end_frame)
                    ]
                    - trialStimStartFrame[tt]
                )
            ...
            if len(manualRewardFrames) > 0:
                ...
                temp_manual_reward = (
                    temp_manual_reward[
                        (temp_manual_reward > temp_start_frame)
                        & (temp_manual_reward < temp_end_frame)
                    ]
                    - trialStimStartFrame[tt]
                )
            if len(quiescentViolationFrames) > 0:
                ...
                temp_quiescent_viol = (
                    temp_quiescent_viol[
                        (temp_quiescent_viol > temp_start_frame)
                        & (temp_quiescent_viol < temp_end_frame)
                    ]
                    - trialStimStartFrame[tt]
                )
            ...
            if len(trialOptoVoltage) > 0:
                if ~np.isnan(trialOptoVoltage[tt]):
                    ...
                    temp_patch = matplotlib.patches.Rectangle(
                        [0, temp_count], 60, 1, color=[0.2, 0.2, 0.8], alpha=0.15
                    )
                    ax[temp_ax].add_patch(temp_patch)

            ax[temp_ax].vlines(
                temp_licks, ymin=temp_count, ymax=temp_count + 1, color="grey"
            )
            ax[temp_ax].vlines(
                temp_reward,
                ymin=temp_count,
            ...
            ax[temp_ax].plot(
                temp_quiescent_viol,
                np.ones(len(temp_quiescent_viol)) * temp_count + 0.5,
                "m*",
            )
        ...
        vis_miss_trials.append(
            (vis_go_non_auto_reward - vis_hit_count) - np.sum(vis_miss_trials)
        )
        vis_correct_reject_trials.append(
            (vis_nogo_count - vis_fa_count) - np.sum(vis_correct_reject_trials)
        )
        vis_autoreward_trials.append(
            (vis_go_count - vis_go_non_auto_reward) - np.sum(vis_autoreward_trials)
        )
        ...
        aud_miss_trials.append(
            (aud_go_non_auto_reward - aud_hit_count) - np.sum(aud_miss_trials)
        )
        aud_correct_reject_trials.append(
            (aud_nogo_count - aud_fa_count) - np.sum(aud_correct_reject_trials)
        )
        aud_autoreward_trials.append(
            (aud_go_count - aud_go_non_auto_reward) - np.sum(aud_autoreward_trials)
        )
        ...
        temp_vis_hit_title = (
            " " + str(bb + 1) + f":{(vis_hit_trials[bb]/vis_go_trials[bb])*100:.1f}%"
        )
        ...
        temp_vis_fa_title = (
            " "
            + str(bb + 1)
            + f":{(vis_false_alarm_trials[bb]/vis_nogo_trials[bb])*100:.1f}%"
        )
        vis_fa_title = vis_fa_title + temp_vis_fa_title

        temp_catch_title = (
            " " + str(bb + 1) + f":{(catch_resp_trials[bb]/catch_trials[bb])*100:.1f}%"
        )
        catch_title = catch_title + temp_catch_title

        temp_aud_hit_title = (
            " " + str(bb + 1) + f":{(aud_hit_trials[bb]/aud_go_trials[bb])*100:.1f}%"
        )
        ...
        temp_aud_fa_title = (
            " "
            + str(bb + 1)
            + f":{(aud_false_alarm_trials[bb]/aud_nogo_trials[bb])*100:.1f}%"
        )
...
def plot_lick_raster_by_block(
    ...
    lick_times_by_trial = tuple(
        lick_times[slice(*start_stop)]
        for start_stop in np.searchsorted(
            lick_times, trials.select("start_time", "stop_time")
        )
    )
    trials = (
        trials.with_columns(
    ...
    response_window_start_time = np.median(
        np.diff(trials.select("stim_start_time", "response_window_start_time"))
    )
    response_window_stop_time = np.median(
        np.diff(trials.select("stim_start_time", "response_window_stop_time"))
    )
    ...
    for ax, stim_name in zip(axes, ("vis1", "sound1")):
        ...
        for idx, trial in enumerate(stim_trials.iter_rows(named=True)):
            ...
            # block label
            if idx_in_block == (
                len(stim_trials.filter(pl.col("block_index") == trial["block_index"]))
                // 2
            ):
                text_color = "black" if is_vis_block == is_vis_stim else "grey"
                rotation = 0
                label = "V" if "vis" in trial["rewarded_modality"] else "A"
                ax.text(
                    -0.4,
                    idx,
                    label,
            ...
            if trial["trial_index_in_block"] < previous_trial_idx:
                ax.axhline(idx - 0.5, **line_params)
                idx_in_block = 0
            ...
            ax.scatter(
                colored_time,
                idx,
                **scatter_params | {"color": first_lick_color, "alpha": 1},
            )
