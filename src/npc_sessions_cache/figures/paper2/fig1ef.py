...
#%%
def format_ax(ax, ax_idx:int | None, is_switch_to_rewarded:bool, preTrials:int, postTrials:int, annotate_rewarded: bool, annotate_context: tuple[str, str] = ()) -> None:
    ax.axvline(x=0,color='grey',lw=.5)
    if is_switch_to_rewarded:
        ax.axvspan(xmin=0,xmax=5,color=[.9, .95, .9], lw=0, zorder=-1)
    ...
    if annotate_rewarded:
        ...
        for i, (annotation, color, x) in enumerate(zip(annotations, colors, (0,0))):
            ax.text(
                x, 1.1, annotation, color=color, fontsize=6,
                va='center', ha=('right' if i==0 else 'left')
            )
    if annotate_context:
        for i, (annotation, color, x) in enumerate(zip(annotate_context, 'kk', (-preTrials/2,postTrials/2))):
            ax.text(
                x, 1.1, annotation, color=color, fontsize=8,
                va='center', ha='center',
            )
...
def plot_suppl(combined: bool = False, late_autorewards: bool | None = None):
    ...
    trials_df = (
        utils.get_prod_trials(cross_modal_dprime_threshold=1.5, late_autorewards=late_autorewards)
        .filter(
    ...
    for rewardStim,blockLabel in zip(('vis1','sound1'),('visual rewarded blocks','auditory rewarded blocks')):
        for figStimName in figStimNames:
            ...
            for stim,stimLbl,clr,ls in zip(stimNames,stimLabels,'ggmm',('-','--','-','--')):
                ...
                ax.plot(x,m,color=clr,label=stimLbl,ls=ls,lw=.3,zorder=99)
                is_sem = True
                if is_sem:
                    s = np.nanstd(y,axis=0)/(len(y)**0.5)
                    ax.fill_between(x,m+s,m-s,color=clr,alpha=.1, edgecolor='none', zorder=50)
                else:
                    y= np.array(y)
                    ...
                    for i in range(len(m)):
                        ...
                        lower[i],upper[i] = np.percentile([np.nanmean(np.random.choice(ys,size=ys.size, replace=True)) for _ in range(1000)],(5, 95))
                    ax.fill_between(x,upper,lower,color=clr,alpha=.1, edgecolor='none', zorder=50)
                is_switch_to_rewarded = (figStimName == rewardStim and not combined) 
                if is_switch_to_rewarded or stim == rewardStim:
                    ax.plot(x[preTrials+1:preTrials+6],m[preTrials+1:preTrials+6],'.',color=clr,label='instruction trials' if late_autorewards==False else 'scheduled reward',ms=1,zorder=99)

            is_switch_to_vis = 'visual' in blockLabel
            annotate_context = ('A','V') if is_switch_to_vis else ('V','A')
            format_ax(ax, 0, False if combined else is_switch_to_rewarded , preTrials, postTrials, False, annotate_context)
            if 'visual' in blockLabel:
                xmin, xmax = 0, postTrials
            else:
                xmin, xmax = -preTrials, 0
            ax.axvspan(xmin,xmax,color=[.95]*3, lw=0, zorder=-99, clip_on=False)
            ...
            figsave_path = pyfile_path.with_name(f"{pyfile_path.stem}_suppl_{autorewards_name}_{'to-vis' if 'visual' in blockLabel else 'to-aud'}_{stim_name}")
...
def plot(late_autorewards: bool | None = None):
    ...
    for ax_idx, (ax, stimLbl,clr) in enumerate(zip(axes, ('rewarded target stim','unrewarded target stim'),'kk')):
        ...
        for subject_id, subject_df in trials_df.group_by(['subject_id']): 
            ...
            for session_id, session_df in subject_df.group_by(['session_id']):
                ...
                for blockInd in np.unique(trialBlock): # range(1,6):
                    ...
                    if blockInd > 0:# and rewStim == blockRewardStim: #! blockRewardStim is defined in the previous cell
                        stim = nonRewStim if 'unrewarded' in stimLbl else rewStim
                        trials = (trialStim==stim) #& ~autoReward
                        y[-1].append(np.full(preTrials+postTrials+1,np.nan))
                        pre = trialResp[(trialBlock==blockInd-1) & trials] # & ~autoReward makes no difference
                        i = min(preTrials,pre.size)
                        y[-1][-1][preTrials-i:preTrials] = pre[-i:]
                        post = trialResp[(trialBlock==blockInd) & trials]
                        i = min(postTrials,post.size)
        ...
        m = np.nanmean(y,axis=0)
        ax.plot(x,m,color=clr,label=stimLbl,lw=.3,zorder=99)
        if is_switch_to_rewarded:
            ax.plot(x[preTrials+1:preTrials+6],m[preTrials+1:preTrials+6],'.',color=clr,label='instruction trials' if late_autorewards==False else 'scheduled reward',ms=2,zorder=99)
        is_sem = False
        if is_sem:
            s = np.nanstd(y,axis=0)/(len(y)**0.5)
            ax.fill_between(x,m+s,m-s,color=clr,alpha=.1, edgecolor='none', zorder=50)
...
if __name__ == "__main__":
    plot_suppl(combined=True)
    plot_suppl(combined=False)
    plot()